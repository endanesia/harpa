		...
		<div class="col-md-12">
			...
			<table class="table table-striped" id="example1">
				...
				<tr>
						...
						<td>
							<?php 
								$this->db->select("sum(jml) as total, count(id) as jml");
								$this->db->from("tbRwThr");
								$this->db->where("id_unit", $unit->id);
								$this->db->where("bulan", $this->session->filter_bulan);
								$this->db->where("tahun", $this->session->filter_tahun);
								$dt = $this->db->get()->result();
								$thn = $this->session->filter_tahun;
								$bln = $this->session->filter_bulan;
								if (isset($dt[0]->total)) {
									echo "Rp. " . number_format($dt[0]->total, 0, ',', '.');
									echo "<a href='". site_url("gaji/thr/?skpd=$unit->id&bulan=$bln&tahun=$thn") ."'>( " . $dt[0]->jml . " personel)</a>";
								}
							?>
						</td>
					</tr>
